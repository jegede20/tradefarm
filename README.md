...
These checks reduce exposure to thin liquidity, creator concentration, one-wallet manipulation, unverified pools, pump chasing, and pools without observed two-way execution. They cannot guarantee profit or prove that a token will never rug; public-chain heuristics can be evaded and market conditions can change after entry. Manual mode resolves only the exact token address entered by the user and skips scanning, while retaining the graduated-pool, creator, LP, liquidity, token-depth, and impact safety gates. Entry size is capped by wallet balance, configured reserve share, and estimated pool impact.
