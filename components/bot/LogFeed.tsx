...
export function LogFeed() {
  ...
  return (
    <section className="panel flex min-h-[500px] flex-col overflow-hidden rounded-lg lg:min-h-[680px]">
      <div className="flex h-[65px] items-center justify-between border-b border-border px-4">
        <div><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-sm bg-accent-primary" /><h2 className="font-mono text-[10px] font-semibold tracking-wider text-text-primary">BOT / EVENT STREAM</h2></div><p className="mt-1.5 font-mono text-[8px] uppercase tracking-wider text-text-secondary">{status === 'running' ? 'PROCESS ACTIVE' : 'PROCESS IDLE'} · {logs.length}/200 LINES</p></div>
