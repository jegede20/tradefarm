...
export function LeaderboardTable({ rows, refresh, isLoading, lastUpdated, source }: { rows: LeaderboardRow[]; refresh: () => void; isLoading: boolean; lastUpdated: number | null; source: 'live' | 'unavailable' }) {
  const { address } = useAccount()
  return (
    <section className="panel overflow-hidden rounded-lg">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-4 py-3.5">
        <div>
          <div className="flex items-center gap-2"><h2 className="text-sm font-semibold">Top Arc traders</h2><span className={cn('rounded-full border px-2 py-0.5 font-mono text-[8px]', source === 'live' ? 'border-success/30 bg-success/10 text-success' : 'border-border bg-bg-elevated text-text-secondary')}>{source === 'live' ? 'LIVE' : 'NO DATA'}</span></div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[760px] border-collapse text-left">
          ...
          <tbody>{rows.length === 0 ? (
            <tr><td colSpan={5} className="px-4 py-16 text-center text-xs text-text-secondary">{isLoading ? 'Reading Flipt USDC transfers from Arc…' : 'No router-linked transfers were returned. Try refreshing.'}</td></tr>
